...
function FlowDiagram() {
  ...
  const flow = language === 'es'
    ? { collector: ['01 · Messenger', 'Workflow de normalización', 'Actualiza el teléfono del contacto cuando Messenger no lo deja guardado.', 'Teléfono listo'], ghl: ['02 · Customer Replied', 'Workflow de captura', 'Dispara cada mensaje con Contact ID, contenido, nombre, teléfono y canal.', 'Mensaje recibido'], api: ['03 · Backend', 'API de dealerADMIN', 'Guarda el evento, reconstruye la conversación, normaliza y aplica routing.', 'Procesado + auditable'], queue: ['04 · DealerADMIN', 'Cola operativa', 'La ventana y el mínimo de routing determinan cuándo aparece el lead.', 'Acción del operador pendiente'] }
    : { collector: ['01 · Messenger', 'Phone normalization workflow', 'Updates the contact phone when Messenger does not persist it.', 'Phone ready'], ghl: ['02 · Customer Replied', 'Capture workflow', 'Sends every message with Contact ID, content, name, phone, and channel.', 'Message received'], api: ['03 · Backend', 'dealerADMIN API', 'Stores the event, rebuilds the conversation, normalizes, and routes it.', 'Processed + auditable'], queue: ['04 · dealerADMIN', 'Operator queue', 'The timing window and routing minimum decide when the lead appears.', 'Operator action pending'] };
...
function BackendWorkflowDiagram() {
  ...
  const workflows = language === 'es'
    ? [['Workflow 1', 'Normalizar teléfono', 'Messenger · Contact Changed', 'Escribe el número válido en el contacto. No conserva la memoria ni decide el routing.'], ['Workflow 2', 'Capturar cada mensaje', 'Customer Replied', 'Envía Contact ID, mensaje, nombre, teléfono y canal al backend.']]
    : [['Workflow 1', 'Normalize phone', 'Messenger · Contact Changed', 'Writes the valid number to the contact. It does not own memory or routing.'], ['Workflow 2', 'Capture every message', 'Customer Replied', 'Sends Contact ID, message, name, phone, and channel to the backend.']];
  const backendSteps = language === 'es'
    ? [['01', 'Persistir', 'Cada mensaje queda en webhook_events y conversation_messages.'], ['02', 'Normalizar', 'El backend reconstruye la conversación y actualiza el snapshot.'], ['03', 'Ubicar', 'Easterns consulta locations y conserva el algoritmo geográfico.'], ['04', 'Esperar', 'Completo: inmediato. Incompleto: 30 min de 03:00 a 15:00 o 3 h fuera de esa ventana.'], ['05', 'Mostrar', 'DealerADMIN publica el lead cuando cumple el mínimo de routing.']]
    : [['01', 'Persist', 'Every message is stored in webhook_events and conversation_messages.'], ['02', 'Normalize', 'The backend rebuilds the conversation and updates its snapshot.'], ['03', 'Locate', 'Easterns queries locations and keeps the existing geographic algorithm.'], ['04', 'Wait', 'Complete: immediate. Incomplete: 30m from 03:00 to 15:00 or 3h outside that window.'], ['05', 'Show', 'DealerADMIN publishes the lead when it meets the routing minimum.']];
  ...
  return <div className="landing-flow rounded-[10px] border border-white/10 bg-[#17231D]/95 p-4 shadow-[0_22px_60px_rgba(0,0,0,0.28)] sm:p-6">
    <div className="mb-6 flex items-center justify-between gap-4 text-xs"><span className="font-semibold text-[#F1F7F4]">{language === 'es' ? 'Dos workflows delgados en GHL' : 'Two thin GHL workflows'}</span><span className="inline-flex items-center gap-2 text-[#AFC1B9]"><span className="h-2 w-2 rounded-full bg-[#5ED5AA]" />{language === 'es' ? 'El backend conserva la lógica' : 'The backend owns the logic'}</span></div>
...
export default function HomePage() {
  ...
  const workflowCopy = language === 'es'
    ? { eyebrow: 'Workflows en GHL', title: 'GHL captura; dealerADMIN procesa todo.', description: 'Se crean dos workflows simples: uno normaliza el teléfono de Messenger y otro dispara cada Customer Replied. GHL no conserva la memoria de negocio ni decide el routing.', steps: [['01', 'Normalizar teléfono', 'Messenger actualiza Contact.Phone'], ['02', 'Capturar mensaje', 'Customer Replied envía Contact ID y contenido'], ['03', 'Guardar historial', 'La BD registra cada mensaje recibido'], ['04', 'Aplicar reglas', 'Backend normaliza, ubica y calcula la ventana'], ['05', 'Mostrar al dealer', 'Routing y cola quedan bajo control de dealerADMIN']] as Array<[string, string, string]> }
    : { eyebrow: 'GHL workflows', title: 'GHL captures; dealerADMIN processes everything.', description: 'There are two simple workflows: one normalizes the Messenger phone and one fires for every Customer Replied. GHL does not own business memory or routing.', steps: [['01', 'Normalize phone', 'Messenger updates Contact.Phone'], ['02', 'Capture message', 'Customer Replied sends Contact ID and content'], ['03', 'Store history', 'The database records every message'], ['04', 'Apply rules', 'The backend normalizes, locates, and calculates the window'], ['05', 'Show the dealer', 'Routing and queue remain under dealerADMIN']] as Array<[string, string, string]> };
